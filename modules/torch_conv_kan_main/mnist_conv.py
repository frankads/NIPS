...
#—————————————CHANGE————————————————————————
from Fast-AutoAug-Torch.augment import Augmentation
from Fast-AutoAug-Torch.search_policy import train_network
import json
import torch

def setup_fastaa():
    print("yes")
    parser = argparse.ArgumentParser(description='MNIST ConvNet Training with FastAA')
    parser.add_argument('--reduced-size', type=int, default=60000, help='Reduced dataset size for FastAA search')
    parser.add_argument('--epochs', type=int, default=120, help='Number of epochs for FastAA search')
    parser.add_argument('--nfolds', type=int, default=8, help='Number of folds for FastAA search')
    parser.add_argument('--num-trials', type=int, default=200, help='Number of trials for FastAA search')
    args = parser.parse_args()

    fastaa_policy = load_or_search_fastaa_policy(args)
    return FastAAWrapper(fastaa_policy)

def load_or_search_fastaa_policy(args):
    try:
...
def train_and_validate(model, bs, epochs=15, dataset_name='MNIST', model_save_dir="./models",
    ...
    if dataset_name == 'MNIST':
        ...
        #____________________CHANGE________________
        fastaa_wrapper = setup_fastaa()
